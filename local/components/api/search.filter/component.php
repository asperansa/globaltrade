...
if(!is_array($arParams['SELECT_IN_CHECKBOX']))
	$arParams['SELECT_IN_CHECKBOX'] = array();
...
$slider_range_all = $slider_range_replace = $slider_range_price_all = $slider_range_price_replace = '';
...
if($this->StartResultCache(false, ($arParams['CACHE_GROUPS'] === 'N' ? false : $USER->GetGroups())))
{
	...
	while($arProp = $rsProp->Fetch())
	{
		if(in_array($arProp['CODE'], $arParams['PROPERTY_CODE']) && $arProp['PROPERTY_TYPE'] == 'N')
		{
	...
	if(!empty($arNumProps) && $arParams['INCLUDE_JQUERY_UI_SLIDER'])
	{
		foreach($arNumProps as $arNumProp)
		{
			...
			$arSliderFilter['!PROPERTY_'. $arNumProp['CODE']] = false;
			...
			{
				$arNumMaxMin[$arNumProp['CODE']]['MIN'] = $arNumElement['PROPERTY_'. $arNumProp['CODE'] .'_VALUE'];
			}
			...
			while($arNumElement = $rsNumPropValues->Fetch())
			{
				$arNumMaxMin[$arNumProp['CODE']]['MAX'] = $arNumElement['PROPERTY_'. $arNumProp['CODE'] .'_VALUE'];
			}

			//Generate jQuery UI range slider for all inputs
			$slider_range_all .='
			$("#'. $arNumProp['CODE'] .'_RANGE" ).slider({
		        range: true,
		        min: '. intval($arNumMaxMin[$arNumProp['CODE']]['MIN']) .',
		        max: '. intval($arNumMaxMin[$arNumProp['CODE']]['MAX']) .',
		        values: [ '. intval($arNumMaxMin[$arNumProp['CODE']]['MIN']) .', '. intval($arNumMaxMin[$arNumProp['CODE']]['MAX']) .' ],
		        slide: function( event, ui ) {
		            $("#'. $arNumProp['CODE'] .'_LEFT").val(ui.values[ 0 ]);
		            $("#'. $arNumProp['CODE'] .'_RIGHT").val(ui.values[ 1 ]);
		        }
		    });' . "\n";
        ...
        //fb($arResult['arNumMaxMin']);


		//Generate jQuery UI range slider script
		if(!empty($slider_range_all))
		{
			$slider_range_script = '<script type="text/javascript">
				jQuery(document).ready(function($){
					' . $slider_range_all .'
					}); //END Ready
					</script>';

			$APPLICATION->AddHeadString($slider_range_script);
		}
    ...
    if( (!empty($arAclCodes) || in_array('NAME',$arParams['FIELD_CODE'])) && $arParams['IBLOCK_ID'] && $arParams['INCLUDE_AUTOCOMPLETE_PLUGIN'])
    {
        ...
        if(!empty($arAclItems))
        {
            foreach($arAclItems as $arAclItem)
            {
                ...
                {
                    $curVal = $arAclItem['PROPERTY_'. $sAclPropCode .'_VALUE'];

                    //Delete empty values
                    //if(!empty($curVal))
                        //$arPrepareAclItems[$sAclPropCode][] = $curVal;

                    //For multiple rows properties
                    if(!empty($curVal))
                        $arPrepareAclItems[$sAclPropCode][] = str_replace("\r\n",' ',$curVal);
                }
            ...
            if(!empty($arPrepareAclItems))
            {
                foreach($arPrepareAclItems as $sCode=>$v)
                {
                    ...
                    $arResult['AutocompleteInitJS'] .='
                        var '.$sCode.'_acl = '. json_encode($arPrepareAclItems[$sCode]) .';
...
foreach($arParams['FIELD_CODE'] as $field_code)
{
	...
	switch($field_code)
	{
		...
		case 'TAGS':
			if(!is_array($value))
			{
				$field_res = '<input id="'.$field_code.'_acl" class="autocomplite"  type="text" name="' . $name . '" style="' . $arParams['TEXT_WIDTH'] . '" value="' . htmlspecialcharsbx($value) . '" />';
				if(strlen($value) > 0)
		...
		case 'SHOW_COUNTER':
			...
			$field_res .= '<input type="text" name="' . $name . '" style="' . $arParams['NUMBER_WIDTH'] . '" value="' . htmlspecialcharsbx($value_left) . '" '. ($arParams['INCLUDE_PLACEHOLDER'] ? ' placeholder="'. $field_code . GetMessage('PH_OT') .'"':  '') . ' />&nbsp;' . GetMessage('CC_BCF_TILL') . '&nbsp;';
			...
			if(is_array($value) && isset($value['RIGHT']))
				...
				$value_right = '';
			$field_res .= '<input type="text" name="' . $name . '" style="' . $arParams['NUMBER_WIDTH'] . '" value="' . htmlspecialcharsbx($value_right) . '"  '. ($arParams['INCLUDE_PLACEHOLDER'] ? ' placeholder="'. $field_code . GetMessage('PH_DO') .'"':  '') . ' />';
...
foreach($arResult['arrProp'] as $prop_id => $arProp)
{
	...
	switch($arProp['PROPERTY_TYPE'])
	{
		...
		case 'G': // флажки
			...
			{
				if ($arProp['CODE'] == 'STATUS') {
					//list($arProp['VALUE_LIST'][5], $arProp['VALUE_LIST'][1]) = array($arProp['VALUE_LIST'][1], $arProp['VALUE_LIST'][5]);
					unset($arProp['VALUE_LIST'][4]);
				}
				if ($arProp['CODE'] == 'PLOT') {
					$res .= '<div class="filter_item_checkbox"><input type="checkbox" value="" name="" id="1_20" class="main_checkbox"><label for="1_20"><span></span><i>Выбрать все</i></label></div>';
				}
				foreach($arProp['VALUE_LIST'] as $key => $val)
				{
					$res .= '<div class="filter_item_checkbox" '. (($key == 2) ? 'style="display:none;"': '') .'>';
					...
					$res .= ' value="' . htmlspecialcharsbx($key) . '"> ';
					
					$res .= '<label for="' . $arProp['CODE'] . '_' . $key . '"><span></span><i>' . htmlspecialcharsbx(($arProp['CODE'] == 'STATUS' ? GetMessage("LABEL_".$key).'()' : $val)) . '</i>';
			...
			else
			{
				if($arProp['MULTIPLE'] == 'Y') {
					$res .= '<ul>';$resS .= '<select multiple name="' . $name . '[]" size="' . $arParams['LIST_HEIGHT'] . '" class="chosen-select chosen-select-deselect" data-placeholder="' . $arProp['NAME'] . '" style="' . $arParams['SELECT_WIDTH'] . '">';
				} else
					$res .= '<select name="' . $name . '" class="chosen-select chosen-select-deselect" data-placeholder="' . $arProp['NAME'] . '" style="' . $arParams['SELECT_WIDTH'] . '">';

				$resS .= '<option value="">' . $labelAll . '</option>';
				$res .= '<li><a href="/doctors/">Все врачи</a></li>';
				$values = $arProp['VALUE_LIST'];
				foreach($arProp['VALUE_LIST'] as $key => $val)
				{	$res .= '<li><a href="/doctors/?arrFilter_pf[SERVICES][]='.htmlspecialcharsbx($key).'&set_filter=Y&set_filter=Найти">' .htmlspecialcharsbx($val). '</a></li>';
					$resS .= '<option';
					...
					$resS .= ' value="' . htmlspecialcharsbx($key) . '">' . htmlspecialcharsbx($val) . '</option>';
				}
				$res .= '</ul>'; $resS .= '</select>';
			}
		...
		case 'N':
			...
			if(is_array($value) && !empty($value['LEFT']) ) { 
				$value_left_label = number_format($value['LEFT'], 0, '.', ' ' );
				$value_left = $value['LEFT'];
			}
			else
				$value_left = '';
            ...
            $min_value = $value['LEFT'] ? $value['LEFT']: $arResult['arNumMaxMin'][$arProp['CODE']]['MIN'];
			
            $max_value = $value['RIGHT'] ? $value['RIGHT']: $arResult['arNumMaxMin'][$arProp['CODE']]['MAX'];

			$res .= '<span class=""'. (($arProp['CODE'] == 'AREA') ? ' style="display:none;"' : '') .'>';
			$res .= '<input type="hidden" id="'. $arProp['CODE'] .'_LEFT"  name="' . $name . '" style="' . $arParams['NUMBER_WIDTH'] . '" value="' . htmlspecialcharsbx($value_left) . '" '. ($arParams['INCLUDE_PLACEHOLDER'] ? ' placeholder="'.  $min_value .'"':  '') . '  />';
			$res .= '<input type="text" id="'. $arProp['CODE'] .'_LEFT_LABEL"  style="' . $arParams['NUMBER_WIDTH'] . '" readonly="readonly" class="input_count_left" value="' . htmlspecialcharsbx($value_left_label) . '" '. ($arParams['INCLUDE_PLACEHOLDER'] ? ' placeholder="'.  number_format ( $min_value, 0, '.', ' ' ) .'"':  '') . '  />';

			if(strlen($value_left) > 0)
				${$FILTER_NAME}['PROPERTY']['>=' . $arProp['CODE']] = doubleval($value_left);
			$name = $FILTER_NAME . '_pf[' . $arProp['CODE'] . '][RIGHT]';

			if(is_array($value) && !empty($value['RIGHT'])) {
				$value_right_label =  number_format($value['RIGHT'], 0, '.', ' ' );
				$value_right =  $value['RIGHT'];
			}
			else
				$value_right = '';

			$res .= '<input type="hidden" id="'. $arProp['CODE'] .'_RIGHT" name="' . $name . '" style="' . $arParams['NUMBER_WIDTH'] . '" class="input_count_right" value="' . htmlspecialcharsbx($value_right) . '"  '. ($arParams['INCLUDE_PLACEHOLDER'] ? ' placeholder="'. $max_value .'"':  '') . ' />';
			$res .= '<input type="text" id="'. $arProp['CODE'] .'_RIGHT_LABEL" style="' . $arParams['NUMBER_WIDTH'] . '" readonly="readonly" class="input_count_right" value="' . htmlspecialcharsbx($value_right_label) . '"  '. ($arParams['INCLUDE_PLACEHOLDER'] ? ' placeholder="'.  number_format ( $max_value, 0, '.', ' ' ) .'"':  '') . ' />';
			
			//Include range slider for property
			if($arParams['INCLUDE_JQUERY_UI_SLIDER'])
			{
				//include range-slider to input
				$res .= '<div id="'. $arProp['CODE'] .'_RANGE" class="slider_prices"></div>';

				//It's work when user change input values
				$slider_range_replace  .= '
					$("#'. $arProp['CODE'] .'_RANGE").slider({
					    range: true,
					    min: '. ($arResult['arNumMaxMin'][$arProp['CODE']]['MIN']) .',
		                max: '. ($arResult['arNumMaxMin'][$arProp['CODE']]['MAX']) .',
				        values: [ '. ($min_value) .', '.  ($max_value) .' ],
						step: 1000,
				        slide: function( event, ui ) {
				            $("#'. $arProp['CODE'] .'_LEFT_LABEL").val(number_format(ui.values[ 0 ], 0, ".", " "));
							$("#'. $arProp['CODE'] .'_LEFT").val(ui.values[ 0 ]);
				            $("#'. $arProp['CODE'] .'_RIGHT_LABEL").val(number_format(ui.values[ 1 ], 0, ".", " "));
			...
			/*if ($arProp['CODE'] == 'TOTAL')
			$res .= '<div class="filter_item_checkbox ">
													<input type="checkbox" value="" name="" id="1_14">
													<label for="1_14"><span></span><i>Не показывать проданные участки</i></label>
												</div>';*/
	...
	if($res)
	{
		$arResult['ITEMS']['PROPERTY_' . $prop_id] = array(
			'NAME'         => htmlspecialcharsbx(GetMessage("NAME_".$prop_id)),
			'CODE'		   => $arProp['CODE'],
			'INPUT'        => $res,
			//'SELECT'	   => $resS,
			'VALUES'	=> $values,
...
//print_r(${$FILTER_NAME});
$arrHouseFilter = ${$FILTER_NAME};
$arrHouseFilter['PROPERTY']['STATUS'] = array(5); // С домом
$arrBookFilter['PROPERTY']['STATUS'] = array(3); // Бронь
$arrFreeFilter['PROPERTY']['STATUS'] = array(1); // Свободен
$arCount[5] = CIBlockElement::GetList(Array(), $arrHouseFilter, array(), array(), array('ID'));
$arCount[3] = CIBlockElement::GetList(Array(), $arrBookFilter, array(), array(), array('ID'));
$arCount[1] = CIBlockElement::GetList(Array(), $arrFreeFilter, array(), array(), array('ID'));
//echo CIBlockElement::GetList(Array(), $arrHouseFilter, array(), array(), array('ID'));
//echo CIBlockElement::GetList(Array(), $arrBookFilter, array(), array(), array('ID'));
//echo CIBlockElement::GetList(Array(), $arrFreeFilter, array(), array(), array('ID'));
$name  = $FILTER_NAME . '_pf[' . $arResult['arrProp'][3]['CODE'] . ']';
			$value = $arrPFV[$arResult['arrProp'][3]['CODE']];
			//SELECT OR CHECKBOX in params
				//print_r($arResult['arrProp'][3]['VALUE_LIST']);
				//list($arResult['arrProp'][3]['VALUE_LIST'][5], $arResult['arrProp'][3]['VALUE_LIST'][1]) = array($arResult['arrProp'][3]['VALUE_LIST'][1], $arResult['arrProp'][3]['VALUE_LIST'][5]);
					//print_r($arResult['arrProp'][3]['VALUE_LIST'][1]);
				unset($arResult['arrProp'][3]['VALUE_LIST'][4]);
				foreach($arResult['arrProp'][3]['VALUE_LIST'] as $key => $val)
				{
					//if ($key == 1) {$key = 5;} else if ($key == 5) {$key = 1;}
					$ress .= '<div class="filter_item_checkbox" '. (($key == 2) ? 'style="display:none;"': '') .'>';
					$ress .= '<input type="checkbox" name="' . $name . '[]" id="' . $arResult['arrProp'][3]['CODE'] . '_' . $key . '"';
					if(is_array($value))
					{
						if(in_array($key, $value))
					...
					$ress .= ' value="' . htmlspecialcharsbx($key) . '"> ';
					
					$ress .= '<label for="' . $arResult['arrProp'][3]['CODE'] . '_' . $key . '"><span></span><i>' . htmlspecialcharsbx(($arResult['arrProp'][3]['CODE'] == 'STATUS' ? GetMessage("LABEL_".$key).' ('.$arCount[$key].')' : $val)) . '</i>';
...
$arResult['ITEMS']['PROPERTY_3']['INPUT'] = $ress;

/*echo $APPLICATION->IncludeComponent(
				"xpage:iblock.elements.counter",
				"",
				Array(
					"FILTER_NAME" => 'arrHouseFilter',
...
foreach($arResult['arrPrice'] as $price_code => $arPrice)
{
	...
	$res_price .= '<input  id="PRICE_'. $price_code .'_LEFT"  type="text" name="' . $name . '" style="' . $arParams['NUMBER_WIDTH'] . '" value="' . htmlspecialcharsbx($value) . '"  '. ($arParams['INCLUDE_PLACEHOLDER'] ? ' placeholder="'. $arResult['arPriceMaxMin'][$price_code]['MIN'] .'"':  '') . ' />&nbsp;' . GetMessage('CC_BCF_TILL') . '&nbsp;';
	...
	$res_price .= '<input id="PRICE_'. $price_code .'_RIGHT" type="text" name="' . $name . '" style="' . $arParams['NUMBER_WIDTH'] . '" value="' . htmlspecialcharsbx($value) . '"  '. ($arParams['INCLUDE_PLACEHOLDER'] ? ' placeholder="'. $arResult['arPriceMaxMin'][$price_code]['MAX'] .'"':  '') . '  />';
	...
	if($arParams['INCLUDE_JQUERY_UI_SLIDER'])
	{
		...
		//if isset del_filter then we must replace values to default max and min
		$min_price_value = !empty($value_left_price) ? $value_left_price : $arResult['arPriceMaxMin'][$price_code]['MIN'];
		$max_price_value = !empty($value_right_price) ? $value_right_price : $arResult['arPriceMaxMin'][$price_code]['MAX'];

		//It's work when user change input values
		$slider_range_price_replace  .= '
					$("#PRICE_'. $price_code .'_RANGE").slider({
					    range: true,
                        min: '. ($arResult['arPriceMaxMin'][$price_code]['MIN']) .',
				        max: '. ($arResult['arPriceMaxMin'][$price_code]['MAX']) .',
						step: 1000,
...
if (empty(${$FILTER_NAME}['PROPERTY']['STATUS']))
	${$FILTER_NAME}['PROPERTY']['STATUS'] = Array (1);
...
if(!empty($slider_range_replace) &&  $arParams['INCLUDE_JQUERY_UI_SLIDER'])
{
...
if(!empty($slider_range_price_replace) &&  $arParams['INCLUDE_JQUERY_UI_SLIDER'])
{
