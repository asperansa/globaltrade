...
{
    // process POST data
    if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"])) && $this->arParams['AJAX_ID'] == $_REQUEST['bxajaxid'])
    {
        ...
        foreach ($arParams["PROPERTY_CODES_REQUIRED"] as $key => $propertyID)
        {
            ...
            elseif (is_array($propertyValue) && array_key_exists("VALUE", $propertyValue))
            {
                if(strlen($propertyValue["VALUE"]) <= 0) {
                    $bError = true;
                } elseif($arResult["PROPERTY_LIST_FULL"][$propertyID]['PROPERTY_TYPE'] == 'N' && !is_numeric($propertyValue["VALUE"])) {
                    $bError = true;
                }
            }
            elseif (!is_array($propertyValue))
            {
                if(strlen($propertyValue) <= 0) {
                    $bError = true;
                } elseif($arResult["PROPERTY_LIST_FULL"][$propertyID]['PROPERTY_TYPE'] == 'N' && !is_numeric($propertyValue)) {
                    $bError = true;
                }
            }
        ...
        foreach($arParams["PROPERTY_CODES"] as $propID) {
            $propertyValue = intval($propID) > 0 ? $arUpdatePropertyValues[$propID] : $arUpdateValues[$propID];
            if($propertyValue && !$this->validate_property($arResult["PROPERTY_LIST_FULL"][$propID], $propertyValue)) {
                $arResult["ERRORS"][$propID] = $this->get_errors($propID);
            }
        }
        ...
        {
            if (!$APPLICATION->CaptchaCheckCode($_REQUEST["captcha_word_".$arParams['AJAX_ID']], $_REQUEST["captcha_sid_".$arParams['AJAX_ID']]))
            {
